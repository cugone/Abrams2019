...
namespace MathUtils {
    ...
    float Compute1dFractalNoise(float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(distanceFromWest); // Gives rounder (nonlinear) results
        ...
        if(renormalize && totalAmplitude > 0.f)
        {
            ...
            totalNoise = MathUtils::EasingFunctions::SmoothStep<3>(totalNoise);		// Push towards extents (octaves pull us away)
    ...
    float Compute2dFractalNoise(float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.x);
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.y);
            float weightWest = 1.f - weightEast;
    ...
    float Compute3dFractalNoise(float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.x);
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.y);
            float weightAbove = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.z);
            float weightWest = 1.f - weightEast;
            float weightSouth = 1.f - weightNorth;
    ...
    float Compute4dFractalNoise(float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.y);
            float weightAbove = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.z);
            float weightAfter = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromMins.w);
            float weightWest = 1.f - weightEast;
            float weightSouth = 1.f - weightNorth;
            float weightBelow = 1.f - weightAbove;
    ...
    float Compute1dPerlinNoise(float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromWest);
            float weightWest = 1.f - weightEast;
    ...
    float Compute2dPerlinNoise(float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            // Do a smoothed (nonlinear) weighted average of dot results
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromSW.x);
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromSW.y);
            float weightWest = 1.f - weightEast;
            float weightSouth = 1.f - weightNorth;
    ...
    float Compute3dPerlinNoise(float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            // Do a smoothed (nonlinear) weighted average of dot results
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBelowSW.x);
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBelowSW.y);
            float weightAbove = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBelowSW.z);
            float weightWest = 1.f - weightEast;
            float weightSouth = 1.f - weightNorth;
            float weightBelow = 1.f - weightAbove;
    ...
    float Compute4dPerlinNoise(float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed) noexcept
    {
        ...
        for(unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
        {
            ...
            // Do a smoothed (nonlinear) weighted average of dot results
            float weightEast = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBeforeBSW.x);
            float weightNorth = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBeforeBSW.y);
            float weightAbove = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBeforeBSW.z);
            float weightAfter = MathUtils::EasingFunctions::SmoothStep<3>(displacementFromBeforeBSW.w);
            float weightWest = 1.f - weightEast;
            float weightSouth = 1.f - weightNorth;
            float weightBelow = 1.f - weightAbove;
            float weightBefore = 1.f - weightAfter;
