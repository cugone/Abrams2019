...
void RodJoint::notify([[maybe_unused]] TimeUtils::FPSeconds deltaSeconds) noexcept {
    ...
    if(distance < _length) { //Compression
        ...
        if(first_body) {
            ...
            first_body->SetPosition(first_body->GetPosition() + direction_to_second * mass2_ratio);
        }
        if(second_body) {
            ...
            second_body->SetPosition(second_body->GetPosition() + direction_to_first * mass1_ratio);
        }
    }
}
